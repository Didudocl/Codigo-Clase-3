...
const domainEmailValidator = (value, helper) => {
  if (!value.endsWith('@gmail.cl')) {
    return helper.message('El email debe ser del dominio @gmail.cl');
  }
  return value;
};
...
export const userBodyValidation = Joi.object({
  nombreCompleto: Joi.string()
    ...
    .messages({
      ...
      'string.pattern.base':
        'El nombre completo permite solo letras de la a-z.',
    }),
  rut: Joi.string()
    .min(9)
    .max(12)
    .pattern(/^(0?[1-9]|1\d|2[0-9])(\.\d{3}){2}-[\dkK]$|^0?[1-9]\d{6,7}-[\dkK]$/)
    .required()
    .messages({
      ...
      'string.pattern.base':
        'El rut debe ser con el formato xx.xxx.xxx-x o xxxxxxxx-x',
    }),
  email: Joi.string()
    .min(15)
    .max(30)
